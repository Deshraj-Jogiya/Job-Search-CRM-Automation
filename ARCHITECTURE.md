...
- General bullet-prose fabrication detection stays open by design. `tailoring_service._verify_structural_fidelity()` catches an invented company/role/date; two narrow additions (`resume_rules.check_years_claim`/`check_unverified_bare_percentage`) catch an over-claimed years-of-experience figure or an unverified bare percentage. Neither catches a fabricated metric or outcome invented directly in bullet prose -- distinguishing "genuinely rephrased from a real bullet" from "invented from nothing" is a harder problem than a mechanical patch can solve honestly. See [`FUTURE.md`](./FUTURE.md#bullet-level-fabrication-detection).
