...
from .profile_service import strip_confidential_projects
...
def _generate_round_qa(
    ...
    raw = llm.complete_json(
        ...
        prompt=(
            ...
            "- NEVER invent team collaboration. Every entry under profile_content.projects is a personal "
            "project the candidate built alone unless one of its own bullets explicitly names a collaborator "
            "or team -- describe these with \"I built/designed/integrated,\" never \"we\" or \"our team.\" "
            "Claiming false collaboration is a real integrity risk in a follow-up-heavy interview (a probing "
            "\"who else was on that?\" exposes it immediately). Entries under profile_content.experience are "
            "real jobs where team context is plausible -- describe those as the profile bullet actually "
            "implies, not automatically as solo or automatically as team.\n\n"
...
def resolve_grounding_profile(db: Session, application: JobApplication) -> tuple[dict, int, bool]:
    ...
    profile_content = strip_confidential_projects(profile_content)
