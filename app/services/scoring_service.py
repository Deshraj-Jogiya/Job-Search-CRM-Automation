...
_WAGE_LEVEL_FIT_SCORE = {"IV": 20, "III": 15, "II": 8, "I": 3}
_WAGE_LEVEL_FIT_UNKNOWN = 6

_SPONSORSHIP_HISTORY_BY_TIER = {"A": 30, "B": 18, "C": 8, "X": 0}
_SPONSORSHIP_HISTORY_UNKNOWN = 4


def _sponsorship_history_component(company: Company | None) -> dict:
    if company is None:
        raw, contribution = None, _SPONSORSHIP_HISTORY_UNKNOWN
    elif company.is_cap_exempt:
        # A direct 30-point floor regardless of tier -- year-round filing
        # ability outside the lottery is itself a strong positive signal,
        # not just a tiebreaker within a tier (see company_tier.py).
        raw, contribution = f"{company.tier or '?'} (cap-exempt)", 30
    elif company.tier in _SPONSORSHIP_HISTORY_BY_TIER:
        raw, contribution = company.tier, _SPONSORSHIP_HISTORY_BY_TIER[company.tier]
    else:
        raw, contribution = None, _SPONSORSHIP_HISTORY_UNKNOWN
    return {"raw": raw, "weight": _SPONSORSHIP_HISTORY_WEIGHT, "contribution": contribution}


def _wage_level_fit_component(company: Company | None) -> dict:
    level = company.max_wage_level_15xx if company else None
    contribution = _WAGE_LEVEL_FIT_SCORE.get(level, _WAGE_LEVEL_FIT_UNKNOWN)
    return {"raw": level, "weight": _WAGE_LEVEL_FIT_WEIGHT, "contribution": contribution}
...
def compute_score_breakdown(application: JobApplication, settings: GlobalSettings | None = None) -> dict:
    ...
    components = {
        ...
        "sponsorship_history": _sponsorship_history_component(company),
        "wage_level_fit": _wage_level_fit_component(company),
...
def recompute_score_breakdown(db: Session, application: JobApplication) -> dict:
    ...
    breakdown = compute_score_breakdown(application, settings)
